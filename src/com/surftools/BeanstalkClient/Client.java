...
public interface Client {
     ...
     *
     * @param priority is an integer < 2**32. Jobs with smaller priority values will be scheduled before jobs
     * with larger priorities. The most urgent priority is 0; the least urgent priority is 4,294,967,295.
     *
     * @param delaySeconds is an integer number of seconds to wait before putting the job in the ready queue.
     * The job will be in the "delayed" state during this time.
     *
     * @param timeToRun is an integer number of seconds to allow a worker to run this job. This time is
     * counted from the moment a worker reserves this job. If the worker does not delete, release, or bury the
     * job within timeToRun seconds, the job will time out and the server will release the job. The minimum
     * timeToRun is 1. If the client sends 0, the server will silently increase the timeToRun to 1.
     *
     * @param data is the job body -- a sequence of bytes
     *
    ...
    /**
     * The "use" command is for producers. Subsequent put commands will put jobs into the tube specified by
     * this command. If no use command has been issued, jobs will be put into the tube named "default".
     *
     * @param tubeName
     */
    ...
    // ****************************************************************
    /**
     * This will return a newly-reserved job. If no job is available to be reserved, beanstalkd will wait to
     * send a response until one becomes available. Once a job is reserved for the client, the client has
     * limited time to run (TTR) the job before the job times out. When the job times out, the server will put
     * the job back into the ready queue. Both the TTR and the actual time left can be found in response to
     * the stats-job command.
     *
     *
     *
     * @param timeoutSeconds A timeout value of 0 will cause the server to immediately return either a
     * response or TIMED_OUT. A positive value of timeout will limit the amount of time the client will block
    ...
    /**
     *
     * The delete command removes a job from the server entirely. It is normally used by the client when the
     * job has successfully run to completion. A client can delete jobs that it has reserved, ready jobs,
     * delayed jobs, and jobs that are buried.
     *
     * @param jobId
     *
    ...
    /**
     * The release command puts a reserved job back into the ready queue (and marks its state as "ready") to
     * be run by any client. It is normally used when the job fails because of a transitory error.
     *
    ...
    /**
     * The bury command puts a job into the "buried" state. Buried jobs are put into a FIFO linked list and
     * will not be touched by the server again until a client kicks them with the "kick" command.
     *
     * @param jobId is the job id to release.
     * @param priority is a new priority to assign to the job.
     *
     ...
     * potentially take a long time, but you still want the benefits of a TTR pulling a job away from an
     * unresponsive worker. A worker may periodically tell the server that it's still alive and processing a
     * job (e.g. it may do this on DEADLINE_SOON). The command postpones the auto release of a reserved job
     * until TTR seconds from when the command is issued.
     *
     * @param jobId is the ID of a job reserved by the current connection.
     *
    ...
    // ****************************************************************
    /**
     * The "watch" command adds the named tube to the watch list for the current connection. A reserve command
     * will take a job from any of the tubes in the watch list. For each new connection, the watch list
     * initially consists of one tube, named "default".
     *
     * @param tubeName is a name at most 200 bytes. It specifies a tube to add to the watch list. If the tube
    ...
    /**
     * The "ignore" command is for consumers. It removes the named tube from the watch list for the current
     * connection.
     *
     * @param tubeName the name of the tube to be ignored.
     *
     * @return the integer number of tubes currently in the watch list, or -1, if the client attempts to
     * ignore the only tube in its watch list.
     */
    ...
    /**
     * The kick command applies only to the currently used tube. It moves jobs into the ready queue. If there
     * are any buried jobs, it will only kick buried jobs. Otherwise it will kick delayed jobs
     *
     * @param count is an integer upper bound on the number of jobs to kick. The server will kick no more than
     * <bound>
     * jobs.
     *
    ...
    /**
     * The kick-job command is a variant of kick that operates with a single job identified by its job id. If
     * the given job id exists and is in a buried or delayed state, it will be moved to the ready queue of the
     * the same tube where it currently belongs.
     *
     * @param jobId is the job id to kick.
     *
     ...
     *
     * @param tubeName is a name at most 200 bytes. Stats will be returned for this tube.
     *
     * @return a Map with the following keys
     *
     ...
     *
     * - "current-workers" is the number of open connections that have each issued at least one reserve
     * command.
     *
     ...
     *
     * - "binlog-max-size" is the maximum size in bytes a binlog file is allowed to get before a new binlog
     * file is opened.
     *
     ...
     * @param pause is an integer number of seconds to wait before reserving any more jobs from the queue
     *
     ...
     * is the client using one unique connection per thread or a single shared connection?
     *
     * @param uniqueConnectionPerThread true for a unique connection per thread, false for a single shared
     * connection
     */
